 ...
 */
export const provisionNewSchool = functions.https.onCall(async (data: any, context) => {
  ...
  if (!nome || !cnpj || !responsavel || !email || !userUid) {
    ...
    if (!responsavel) camposFaltando.push('responsavel');
    if (!email) camposFaltando.push('email');
    if (!userUid) camposFaltando.push('userUid');
